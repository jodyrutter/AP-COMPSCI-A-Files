...
public class CaesarShiftEncryption
{
    ...
    {
        System.out.println();
        System.out.print("Enter a message to be translated: ");
    ...
    public static void encryptedMessage()
    {
        ...
        for(int index = 0; index<translateInput.length() ; index++)
        {
            ...
            {
                if(translateInput.substring(index, index+1).equalsIgnoreCase(cipherAlphabet[spot]))
                {
                    output += regularAlphabet[spot];
                }
            }
        }
        System.out.println("The encrypted version is \""+output+"\"");
        System.out.println();
        System.out.print("Would you like to quit? (y/n) ");
        String terminate = in.nextLine();
        if(terminate.substring(0,1).equalsIgnoreCase("y"))
        {
            System.exit(0);
        }
        else
